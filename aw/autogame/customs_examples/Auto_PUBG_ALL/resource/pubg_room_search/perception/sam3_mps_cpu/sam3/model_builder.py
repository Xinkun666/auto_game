...
def normalize_device_string(device=None):
    """Return a torch device string without dropping CUDA device indexes."""
    return str(get_device(device))
...
def get_device_info(device=None):
    ...
    return (dev.type == "cuda", dev.type == "mps", dev.type == "cpu", dev)
...
def build_tracker(
    ...
    if with_backbone:
        vision_backbone = _create_vision_backbone(
            compile_mode=compile_mode, device=None
        )
...
def _setup_device_and_mode(model, device, eval_mode):
    ...
    if device_obj.type == "cuda":
        ...
        try:
            ...
            print(
                f"Warning: MPS not available or error occurred, falling back to CPU. Error: {e}"
            )
            model = model.cpu()
