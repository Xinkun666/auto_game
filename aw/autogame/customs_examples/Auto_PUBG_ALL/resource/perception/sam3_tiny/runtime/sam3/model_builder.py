...
def _load_checkpoint_safe(model, checkpoint_path):
    """Load checkpoint with size-mismatch tolerance (for ctx-length differences)."""
    with g_pathmgr.open(checkpoint_path, "rb") as f:
        try:
            ckpt = torch.load(f, map_location="cpu", weights_only=True)
        except TypeError:
    ...
    # Filter out keys with size mismatch (e.g. pos_embed with different ctx length)
    sam3_image_ckpt = {}
    for k, v in cleaned_ckpt.items():
        if k in model_state:
            ...
            # For backbone keys not in model_state, try to match
            sam3_image_ckpt[k] = v
...
def build_efficientsam3_image_model(
    ...
    # Create text components
    if text_encoder_type:
        # Init text encoder at the target context length to match checkpoint pos-embed
        text_encoder = _create_student_text_encoder(
            bpe_path, text_encoder_type, context_length=text_encoder_context_length
        )
    else:
        text_encoder = _create_text_encoder(bpe_path)
