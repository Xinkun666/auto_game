...
from aw.autogame.customs_examples.Game_Recording import info as exported_info
from aw.autogame.customs_game_examples.Game_Recording import info as local_info
...
def _select_info_module():
    """优先读取 Label 导出的布局，尚未导出时读取本目录的 info.py。"""
    exported_stage_info = getattr(exported_info, "STAGE_INFO", {})
    if isinstance(exported_stage_info, dict) and exported_stage_info:
        return exported_info
    return local_info


info = _select_info_module()
SPECIAL_KEYS = {
