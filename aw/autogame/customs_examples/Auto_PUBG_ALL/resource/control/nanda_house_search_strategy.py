...
未配置南大方案时，策略返回 ``DISABLED``。生产 R 城搜房使用
``exclusive`` 模式：只有南大回放完成并通过出房复核才记为搜房完成。
门框取景或房型匹配没有可信结果属于业务上的 ``NO_MATCH``，只跳过
当前入门点；依赖、程序、回放或回放后复核异常会禁用本轮南大管线，
人物在室外时跳过当前入门点，已在室内时按意外进房直接出房，不回退旋转搜房。
"""
