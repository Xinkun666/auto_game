...
def _build_on_stage_template(stage_names: List[str], typed: bool = True) -> str:
    lines = (
        [
...
def _build_script_autocomplete_types(stages: List["StageData"]) -> str:
    """Build project-specific editor hints without changing the runtime worker."""
    stage_names = []
    info_names = []
    control_names = []
    stage_seen = set()
    ...
    for stage in stages:
        add(stage_names, stage_seen, stage.name)
    for stage in stages:
        for scene in stage.scenes:
            for item in scene.items:
                if item.item_type in {"area", "special_area"}:
                    add(info_names, info_seen, item.name)
                elif item.item_type == "control":
                    add(control_names, control_seen, item.name)

    def literal(values):
        return "Literal[" + ", ".join(repr(value) for value in values) + "]" if values else "str"

    return "\n".join((
        "# Auto-generated by Label export; used only by Python editors.",
        "from typing import Any, List, Literal, Optional, Protocol, Tuple, Union",
        "",
        f"StageName = {literal(stage_names)}",
        f"InfoName = {literal(info_names)}",
        f"ControlName = {literal(control_names)}",
...
class AutoStudioWindow(QMainWindow):
    ...
    def export_project(self):
        ...
        try:
            ...
            if scripts_source:
                ...
                try:
                    with open(template_path, "r", encoding="utf-8") as template_file:
                        is_unmodified_legacy_template = template_file.read() == _build_on_stage_template(
                            [stage.name for stage in self.project.stages], typed=False
                        )
                except OSError:
                    is_unmodified_legacy_template = False
                if is_unmodified_legacy_template:
                    ...
                    try:
                        ...
                        pass
                legacy_hints_path = os.path.join(scripts_dir, "_autocomplete.py")
                try:
                    with open(legacy_hints_path, "r", encoding="utf-8") as legacy_hints_file:
                        is_generated_legacy_hints = legacy_hints_file.readline().strip() == (
                            "# Auto-generated by Label export; used only by Python editors."
                        )
                    if is_generated_legacy_hints:
                        os.remove(legacy_hints_path)
                except OSError:
                    pass
            else:
            ...
            code_lines.append("\n# Script editor hints")
            code_lines.extend(_build_script_autocomplete_types(self.project.stages).splitlines())
